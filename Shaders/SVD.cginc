...
#ifndef __GS_SVD_CGINC__
#define __GS_SVD_CGINC__
...
void GetSVD3D(in float3x3 A, out float3x3 U, out float3 D, out float3x3 V)
{
    ...
    [unroll(5)]
    while (count < 5 && abs(beta[1]) > tao && abs(beta[0]) > tao && abs(alpha[0]) > tao && abs(alpha[1]) > tao && abs(alpha[2]) > tao)
    {
